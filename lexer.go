...
func (x *xfccDefinition) Lex(filename string, reader io.Reader) (lexer.Lexer, error) {
  return &xfccLexer{
...
func (x *xfccDefinition) Symbols() map[string]lexer.TokenType {
  return map[string]lexer.TokenType{
    "EOF":    lexer.TokenType(tokenEOF),
    "String": lexer.TokenType(tokString),
    "Char":   lexer.TokenType(tokChar),
  }
}
...
func (x *xfccLexer) Next() (lexer.Token, error) {
  ...
  for {
    ...
    if escaping {
      ...
      if quoting {
        quoting = false
        return token(lexer.TokenType(tokString)), nil
      } else {
        quoting = true
      }
    } else if !quoting && strings.ContainsRune(x.separators, char) {
      ...
      x.buf.WriteRune(char)
      return token(lexer.TokenType(tokChar)), nil
    } else {
      x.buf.WriteRune(char)
    }
  }
